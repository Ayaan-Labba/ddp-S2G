...
│   ├── special_tokens.py      # Vocabulary special tokens (<extra_id_3>, <extra_id_4>, <extra_id_6> …)
...
Each entity block is introduced by the fixed entity token `<extra_id_3>`. Relation targets feature the static `<extra_id_8>` tail token directly followed by tail entity text. Entities without outgoing relations in joint variants simply omit relation tokens. RE variants omit non-head entities.
