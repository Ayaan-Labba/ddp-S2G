"""
Fine-tuning script for the S2G model on benchmark datasets.

Loads a Flan-T5 Base checkpoint (pre-trained or vanilla), fine-tunes on
a benchmark dataset (CoNLL04, NYT-multi, or SciERC), and evaluates using
the sequential pipeline.

Usage::

    # Fine-tune on CoNLL04 from a pre-trained S2G checkpoint
    python -m s2g.scripts.finetune \\
    ...
    # Fine-tune from vanilla Flan-T5 Base (no pre-training)
    python -m s2g.scripts.finetune \\
        --config configs/finetune.yaml \\
        model.name=google/flan-t5-base \\
        data.data_dir=data/conll04 ...

Dataset-specific hyperparameter suggestions
-------------------------------------------
CoNLL04  (~922 train)  : max_steps=10_000, lr=1e-4, warmup=500
NYT-multi (~56K train) : max_steps=60_000, lr=5e-5, warmup=2_000
SciERC   (~5K sent)    : max_steps=20_000, lr=5e-5, warmup=1_000

These are starting points; tune via W&B sweeps.
"""
