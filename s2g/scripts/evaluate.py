...
def _to_entity_data(source_tokens: List[str], entities: List[EntityBlock]) -> List[Tuple[int, int, str]]:
    return list(dict.fromkeys((*span, e["type"]) for e in entities if e.get("type") for span in find_all_token_spans(source_tokens, e["text"])))


def _evaluate_pipeline(model, tokenizer, instances, entity_schema, rel_schema, tokens, max_source_length, max_target_length, batch_size, eval_beams, device, constraint_decoding) -> Tuple[Dict[str, Any], Dict[str, float]]:
    def _run(inputs):
    ...
    n_per_inst = _run([build_ner_encoder_input(entity_schema, inst["tokens"], _to_spans(inst["tokens"], b), False, tokens) for inst, b in zip(instances, b_per_inst)])
    
    ner_maps = [{e["text"]: e.get("type", "") for e in n} for n in n_per_inst]
    r_per_inst = _run([build_re_encoder_input(rel_schema, inst["tokens"], _to_entity_data(inst["tokens"], n), False, tokens) for inst, n in zip(instances, n_per_inst)])

    per_inst, m = [], {}
    for i, inst in enumerate(instances):
        per_inst.append({
            "text": inst["text"], 
            "boundary_spans": [e["text"] for e in b_per_inst[i]],
            "ner_entities": [{"text": e["text"], "type": e.get("type")} for e in n_per_inst[i]],
    ...
    m.update({f"re_{k}": v for k, v in compute_metrics_for_task("re", all_pred_triplets=[extract_triplets(r) for r in r_per_inst], all_gold_triplets=[[(r["head"]["text"], r["type"], r["tail"]["text"]) for r in inst["relations"]] for inst in instances], all_pred_quintuples=[[(e["text"], ner_maps[i].get(e["text"], ""), rel["type"], rel["tail"], ner_maps[i].get(rel["tail"], "")) for e in r_per_inst[i] for rel in e["relations"]] for i in range(len(instances))], all_gold_quintuples=[[(r["head"]["text"], r["head"].get("type",""), r["type"], r["tail"]["text"], r["tail"].get("type","")) for r in inst["relations"]] for inst in instances]).items()})

    return per_inst, m


def _evaluate_joint(model, tokenizer, instances, entity_schema, rel_schema, tokens, max_source_length, max_target_length, batch_size, eval_beams, device, constraint_decoding) -> Tuple[Dict[str, Any], Dict[str, float]]:
    def _run(inputs):
        return [ent for i in tqdm(range(0, len(inputs), batch_size), leave=False) 
                for ent in _generate_batch(model, tokenizer, inputs[i:i+batch_size], tokens, max_source_length, max_target_length, eval_beams, device, constraint_decoding)]
    ...
    for i, inst in enumerate(instances):
        per_inst.append({
            "text": inst["text"],
            "joint_triplets": [{"head": t[0], "type": t[1], "tail": t[2]} for t in extract_triplets(j_per_inst[i])],
            "joint_plus": {"entities": [{"text": e["text"], "type": e.get("type")} for e in jp_per_inst[i]], "triplets": [{"head": t[0], "type": t[1], "tail": t[2]} for t in extract_triplets(jp_per_inst[i])]},
            "gold_triplets": [{"head": r["head"]["text"], "type": r["type"], "tail": r["tail"]["text"]} for r in inst["relations"]]
        })
