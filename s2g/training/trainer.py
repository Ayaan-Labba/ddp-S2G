...
Changes vs. original
--------------------
1. compute_metrics_for_task is now called with rel_schema / entity_schema
   everywhere so that (a) hallucinated types are discarded and (b) macro F1
   is computed per-type in REBEL style rather than per-instance.

2. _compute_metrics_hf: the "re" variant now correctly uses task="re" and
   passes quintuples so that strict (typed) F1 is computed.  The old code
   always called task="boundary_re", silently skipping strict F1 for "re".

3. Schema params are threaded through _evaluate_dataset → compute_metrics_for_task
   for every task branch.
"""
...
def _to_spans(source_tokens: List[str], entities: List[EntityBlock]) -> List[Tuple[int, int]]:
    return list(dict.fromkeys(
        span for ent in entities for span in find_all_token_spans(source_tokens, ent["text"])
    ))


def _to_entity_data(
    source_tokens: List[str], entities: List[EntityBlock], use_type: bool = True
...
def _assemble_re_quintuples(
    re_entities: List[EntityBlock], ner_map: Dict[str, str]
...
class S2GTrainer(Seq2SeqTrainer):
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._specials_to_remove = [
            tok for tok in (
                self.processing_class.pad_token,
    ...
    # ------------------------------------------------------------------
    # Pipeline / boundary-joint evaluation
    # ------------------------------------------------------------------
            ...
            if self._variant in {"pipeline", "boundary_pipeline", "boundary", "ner", "re", "boundary_re"}:
                ...
                if "re" in self._tasks:
                    m.update(compute_metrics_for_task(
                        ...
                        all_pred_quintuples=[
                            ...
                            for r, nm in zip(r_per_inst, ner_maps)
                        ],
                ...
                if "joint" in self._tasks:
                    m.update(compute_metrics_for_task(
                        ...
                        all_gold_entities=g_ents,
                        all_pred_entity_mentions=[
                            [(e["text"], e.get("type", "")) for e in jp if e.get("type")]
    ...
    # ------------------------------------------------------------------
    # Pipeline multi-step inference
    # ------------------------------------------------------------------

    def _get_pipeline_predictions(
        self, instances: List[Dict], prefix: str
    ) -> Dict[str, List]:
        ...
        return {
            ...
            "g_quints": [
                [
                    (
                        r["head"]["text"],
                        r["head"].get("type", "") if (use_ner or use_re) else "",
                        r["type"],
                        r["tail"]["text"],
                        r["tail"].get("type", "") if (use_ner or use_re) else "",
                    )
    ...
    # ------------------------------------------------------------------
    # BoundaryJoint / Joint multi-step inference
    # ------------------------------------------------------------------

    def _get_boundary_joint_predictions(
        self, instances: List[Dict], prefix: str
    ) -> Dict[str, List]:
        ...
        if use_boundary_joint:
            j_inputs   = [
                build_boundary_joint_encoder_input(
                    self._rel_schema, inst["text"], False, self._tokens, ssi_prompt=self._ssi_prompt
                )
                for inst in instances
            ]
        ...
        return {
            ...
            "g_quints": [
                [
                    (
                        r["head"]["text"], r["head"].get("type", ""),
                        r["type"],
                        r["tail"]["text"], r["tail"].get("type", ""),
                    )
                    for r in inst["relations"]
                ]
                for inst in instances
    ...
    def _run_generation(
        self, encoder_inputs: List[str], desc: str = "Generating"
        ...
        for i in tqdm(
            ...
            tok_out = self.processing_class(
                encoder_inputs[i:i + self._eval_bs],
    ...
    # ------------------------------------------------------------------
    # HF-native compute_metrics (non-pipeline variants only)
    # ------------------------------------------------------------------
    ...
    def _compute_metrics_hf(self, eval_preds: Any) -> Dict[str, float]:
        ...
        if self._variant == "boundary_joint":
            ...
            # BUG FIX: the original code always called task="boundary_re", which
            # silently skipped strict (typed) F1 even when the variant is "re".
            # Now:
