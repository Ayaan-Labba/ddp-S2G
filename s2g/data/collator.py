...
class S2GCollator:
    ...
    def __init__(self, 
        ...
        self.markers = config.get('markers', 'fixed')
    ...
    def prepare_re(self, inst: Dict) -> Tuple[str, str]:
        ...
        dec = build_graph(
            blocks, 're', self.tok, 
            nesting=self.nesting, markers=self.markers,
    ...
    def prepare_boundary_joint(self, inst: Dict) -> Tuple[str, str]:
        ...
        blocks = organise_filter_and_block(
            inst['entities'], inst['relations'], set(), set(pos_rel),
            variant='boundary_joint', use_types=False, dedup=self.dedup
        )
        dec = build_graph(
    ...
    def prepare_joint(self, inst: Dict) -> Tuple[str, str]:
        ...
        blocks = organise_filter_and_block(
            inst['entities'], inst['relations'], set(pos_ent), set(pos_rel),
            variant='joint', use_types=True, dedup=self.dedup
        )
        dec = build_graph(
