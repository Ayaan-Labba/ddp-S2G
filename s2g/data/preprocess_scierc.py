    ...
    for i, ner in enumerate(doc.get("ner", [])):
        for s, e, t in ner:
            ...
            if key not in span_to_ent and 0 <= s_abs <= e_abs < len(doc_tokens):
                ent = {"text": " ".join(doc_tokens[key[0]:key[1]]), "offset": list(key), "type": t}
                span_to_ent[key] = ent
                entities.append(ent)
...
def process_split(input_path: Path, sent_out: Path, doc_out: Optional[Path]) -> Tuple[List[str], List[str]]:
    seen_ent: Set[str] = set()
    seen_rel: Set[str] = set()
    sent_written = 0
    
    with open(input_path, encoding="utf-8") as fin, ExitStack() as stack:
        fs = stack.enter_context(open(sent_out, "w", encoding="utf-8"))
        fd = stack.enter_context(open(doc_out, "w", encoding="utf-8")) if doc_out else None
        ...
        for line in filter(None, (ln.strip() for ln in fin)):
            ...
            for inst in convert_document(doc):
                fs.write(json.dumps(inst, ensure_ascii=False) + "\n")
                seen_ent.update(inst["entity_types"])
                seen_rel.update(inst["rel_types"])
                sent_written += 1
                
            # Document-level processing (if enabled)
            if fd and (d_inst := convert_document_level(doc)):
                fd.write(json.dumps(d_inst, ensure_ascii=False) + "\n")
        
    logger.info("%s → %s (%d sentences written)", input_path.name, sent_out.name, sent_written)
    ...
    for in_name, out_name in {"train.json": "train.jsonl", "dev.json": "val.jsonl", "test.json": "test.jsonl"}.items():
        if (in_path := input_dir / in_name).exists():
            e, r = process_split(
                in_path, 
                output_dir / out_name, 
                output_dir / out_name.replace(".jsonl", "_doc.jsonl") if args.document_level else None
            )
